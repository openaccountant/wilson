...
import { PROVIDERS, type Model, type ModelTag } from '../utils/model.js';
import type { ApprovalDecision } from '../agent/types.js';
import { selectListTheme, theme } from '../theme.js';

const TAG_COLORS: Record<ModelTag, (text: string) => string> = {
  paid: chalk.red,
  open: chalk.green,
  local: chalk.green,
  cloud: chalk.blue,
...
function formatTags(tags: ModelTag[]): string {
  ...
  return ' ' + tags.map((tag) => TAG_COLORS[tag](tag)).join(' ');
}
...
export function createModelSelector(
  ...
  const items: SelectItem[] = models.map((model, index) => ({
    value: model.id,
    label: `${index + 1}. ${model.displayName}${model.tags ? formatTags(model.tags) : ''}${currentModel === model.id ? ' ✓' : ''}`,
  }));
